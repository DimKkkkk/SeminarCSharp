...
// -------------------------- Начало программы ПРИ ПОМОЩИ МЕТОДА ----------------------------------
Console.Clear();
int X = 0, Y = 0;
InitCoordinatesByUser(ref X, ref Y); // Вызов метода с передачей параметров по ссылке (ref). Присваиваем переменные с ref!!! ref используем когда нужно вернуть НЕСКОЛЬКо значений. для будущего ввода координат пользователем. Иначе система выдаст нам координаты с нулями.
...
static void InitCoordinatesByUser(ref int x, ref int y)
{
    ...
    catch (Exception exc)
    {
        ...
        return; // вернёт нас к методу из строки 7
    }
}
// -----------------------------------------------------------------------------
...
static void PrintQuarterByCoords(int x, int y)  // static void - это
{
    if (x > 0 && y > 0)
        Console.WriteLine("1");
    else if (x < 0 && y > 0)
